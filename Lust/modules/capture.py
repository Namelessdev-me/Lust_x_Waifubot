...
async def capture(update: Update, context: CallbackContext):
    ...
    now = time.time()
    if user_id in capture_cooldowns:
        remaining = COOLDOWN - (now - capture_cooldowns[user_id])
        if remaining > 0:
            await update.message.reply_text(capsify(f"⏳ Wait {int(remaining)}s before trying again!"))
            return

    if user_id in pending_captures:
        await update.message.reply_text(capsify("❌ You already have an active capture attempt!"))
        return
    ...
    pending_captures[user_id] = character
    capture_cooldowns[user_id] = now
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎯 CAPTURE HER", callback_data=f"capture:{user_id}")]
    ])
...
@block_cbq_ptb
async def capture_callback(update: Update, context: CallbackContext):
    ...
    if clicker_id != owner_id:
        await query.answer("This isn't your capture attempt!", show_alert=True)
        return

    character = pending_captures.pop(owner_id, None)
    if not character:
    ...
    if success:
        ...
        result_text = (
            f"{capsify('😏 YOU CAUGHT HER!')}\n\n"
            f"{capsify('♦️ NAME:')} {capsify(name)}\n"
            f"{capsify('🧧 ANIME:')} {capsify(anime)}\n"
            f"{capsify('🆔:')} {char_id}\n"
            f"{capsify('🌟 RARITY:')} {rarity}\n\n"
            f"{capsify('✅ SUCCESSFULLY CAPTURED!')}"
        )
    else:
        result_text = (
            f"{capsify('💀 SHE SLIPPED AWAY!')}\n\n"
            f"{capsify('♦️ NAME:')} {capsify(name)}\n"
            f"{capsify('🧧 ANIME:')} {capsify(anime)}\n"
            f"{capsify('🆔:')} {char_id}\n"
            f"{capsify('🌟 RARITY:')} {rarity}\n\n"
...
application.add_handler(CallbackQueryHandler(capture_callback, pattern=r"^capture:"))
